...
class AgentDQN(AgentBase):
    ...
    def get_cumulative_rewards(self, rewards: TEN, undones: TEN) -> TEN:
        ...
        #! .argmax(dim=1) 返回的是最大 Q 值对应的动作索引（Action Index）（例如整数 0, 1, 3），而不是 Q 值本身。后果：在循环中 rewards[t] + masks[t] * next_value 时，你实际上是在把“动作索引”当作“未来价值”加到了奖励上。这在数学上是没有意义的（除非你的 Q 值恰好和动作索引范围一致，这几乎不可能）。
