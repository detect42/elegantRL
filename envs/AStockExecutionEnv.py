...
@nb.njit
def _numba_seek_next_valid(time_idx: int,
                           max_steps: int,
                           sum_impact: float,
                           impact_decay: float,
                           vol_arr: np.ndarray,
                           ...
                           signal_1m_norm: np.ndarray, # 保持你的原有参数位
                           signal_30m_row: np.ndarray, # 新增：30m信号 用于判断是否有机会
                           vol_arr_level: np.ndarray, # 新增：1档量 用于判断是否有足够的流动性
                           vol_level1_threshold_p: float, # 新增：1档量阈值
                           side: int,
                           impact_threshold: float,    # 保持你的原有参数位
                           signal_norm_threshold: float, # 保持你的原有参数位
                           signal_impact_k: float,
                           signal_1m_row_offset: float,
                           force_first_step: bool,     # ⚡ 新增：是否强制先走一步
    ...
    if force_first_step:
        ...
        if time_idx >= max_steps:
            return time_idx, sum_impact, add_vol, add_amt, True

    # 判断当前点是否满足交易条件，不满足则快进


    #vol_level_1_threshold = float(np.quantile(vol_arr_level[:time_idx + 1,0], vol_level1_threshold_p)) if time_idx > 10 else 0.0
    #vol_level_1_threshold = float(np.quantile(vol_arr_level[:,0], vol_level1_threshold_p))
    #!while signal_1m_row[time_idx] < side * sum_impact * signal_impact_k + signal_1m_row_offset: #or  vol_arr_level[time_idx,0] < vol_level_1_threshold:
    while signal_1m_row[time_idx] < side * sum_impact * signal_impact_k + signal_1m_row_offset: #or signal_30m_row[time_idx] * side < 0: 
    #while signal_30m_row[time_idx] * side < side * sum_impact * signal_impact_k + signal_1m_row_offset: #or  vol_arr_level[time_idx,0] < vol_level_1_threshold:
        #if vol_level_1_threshold > vol_arr_level[time_idx,0]:  # 如果1档量都不满足，直接快进到满足的点，避免无效等待
        #    print("1level:",vol_arr_level[time_idx,0],"threshold:",vol_level_1_threshold,"min=",np.min(vol_arr_level[:time_idx + 1,0]))
        time_idx += 1
        sum_impact *= impact_decay
        add_vol += vol_arr[time_idx]
        add_amt += amount_arr[time_idx]
        if time_idx >= max_steps:
            terminated = True
            break
        if force_first_step==False and time_idx == max_steps - 1:
            break
        #vol_level_1_threshold = float(np.quantile(vol_arr_level[:time_idx + 1,0], vol_level1_threshold_p)) if time_idx > 10 else 0.0
...
class AStockExecutionEnv:
    def __init__(self, cfg: DictConfig):
        ...
        assert len(self.STATE_FEATURES) == self.state_dim, \
            f"YAML 中的 state_dim ({self.state_dim}) 与 STATE_FEATURES 的长度 ({len(self.STATE_FEATURES)}) 不一致！"
    ...
    def reset(self, set_id: int = -1,mode: str = "train") -> Tuple[ARY, dict]:
        ...
        while True:
            ...
            self.time_idx, self.sum_impact, add_vol, add_amt, terminated = _numba_seek_next_valid(
                self.time_idx,
                self.max_steps,
                self.sum_impact,
                self._impact_decay,
                ...
                self._30m_ret_raw_arr,
                self._ask_vol_arr if self.side > 0 else self._bid_vol_arr,
                self.vol_level1_threshold_p,
                self.side,
                self.impact_threshold,
                self.signal_norm_threshold,
                self.signal_impact_k,
                self.signal_1m_row_offset,
                force_first_step=False  # 起点不需要强制走一步
            )

            self.global_cum_volume += add_vol
            self.global_cum_amount += add_amt

            # 如果刚出生就被快进到了全天结束，说明这个 case 全天都没机会，直接重抽！
            if terminated:
                #print(f"Case {self.case_info['file_path']} 全天无交易机会，已跳过。")
                if set_id != -1:
                    #print(f"Case {self.case_info['file_path']} 全天无交易机会，已跳过。")
                    self.case_info["skipped"] = True
                    break
                continue
                
            # 找到了健康的起点，打破循环，正常走后面的逻辑
            break
    ...
    with torch.no_grad():
        for _ in range(500000000):
            ...
            if terminated or truncated:
                ...
                #·print("-" * 50)
                id += 1
                if id >= 1: #len_test: # 跑完我们拉取的所有 test_files 就结束
                    print(f"[Case {id:04d}] [Step {_}] slippage={-reward_cum:.6f}, mean={np.mean(slippage_pnl):.6f}",info["actual_total_amount"], info["ideal_total_amount"])
