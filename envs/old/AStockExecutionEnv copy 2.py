...
    #while 1==-1:
    #while sum_impact > 0.0001 and signal_1m_row[time_idx] < side * sum_impact * 8:
    #while sum_impact * side > impact_threshold:
    while signal_1m_norm[time_idx] <= signal_norm_threshold or signal_1m_row[time_idx] < side * sum_impact * 8: 
        time_idx += 1
        sum_impact *= impact_decay
        add_vol += vol_arr[time_idx]
        add_amt += amount_arr[time_idx]
...
        print(f"Fast-forwarded to time_idx {self.time_idx} with signal_1m_norm={signal_1m_norm[self.time_idx]:.4f}, signal_1m={signal_1m_row[self.time_idx]:.4f}, sum_impact={self.sum_impact:.6f}")
...
    actor_path = "/home/songrui.wang/RL_training/log/20260330-221945_d0.04_m0/actor__000542638080_-0061.538.pt"
...
                print(f"[Case {id:04d}] [Step {_}] slippage={-reward_cum:.6f}, mean={np.mean(slippage_pnl):.6f}")
            
            id += 1
...
我发现我需要在reset阶段就需要一次_numba_fast_forward 不然总会从第一步执行（哪怕第一步不满足触发要求）
