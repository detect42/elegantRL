...
def check_stock_trading_env():
    ...
    case_start_t = time.perf_counter()
    print(info)
    ...
    for _ in range(500000000):
        ...
        if terminated or truncated:
            ...
            case_run_s = time.perf_counter() - case_start_t
            case_l2_s = env.case_timing["compute_l2_execution_s"]
            case_forced_s = env.case_timing["compute_forced_execution_cost_s"]
            case_other_s = max(0.0, case_run_s - case_l2_s - case_forced_s)
            case_read_s = env.last_read_file_time

            cum_read_s += case_read_s
            cum_case_run_s += case_run_s
            cum_l2_s += case_l2_s
            cum_forced_s += case_forced_s
            cum_other_s += case_other_s

            case_total_s = case_read_s + case_run_s
            cum_total_s = cum_read_s + cum_case_run_s
            ...
            if random.random() < 1:
                ...
                print(
                    ...
                    f"run={case_run_s:.6f}s ({_safe_pct(case_run_s, case_total_s):.2f}%), "
                    f"run_split: l2={case_l2_s:.6f}s ({_safe_pct(case_l2_s, case_run_s):.2f}%), "
                    f"forced={case_forced_s:.6f}s ({_safe_pct(case_forced_s, case_run_s):.2f}%), "
                    f"other={case_other_s:.6f}s ({_safe_pct(case_other_s, case_run_s):.2f}%)"
                )
                print(
                    f"[cum  {id + 1:04d}] read={cum_read_s:.6f}s ({_safe_pct(cum_read_s, cum_total_s):.2f}%), "
                    f"run={cum_case_run_s:.6f}s ({_safe_pct(cum_case_run_s, cum_total_s):.2f}%), "
                    f"run_split: l2={cum_l2_s:.6f}s ({_safe_pct(cum_l2_s, cum_case_run_s):.2f}%), "
                    f"forced={cum_forced_s:.6f}s ({_safe_pct(cum_forced_s, cum_case_run_s):.2f}%), "
                    f"other={cum_other_s:.6f}s ({_safe_pct(cum_other_s, cum_case_run_s):.2f}%)"
                )
            # print("-----------end-----------")
            id += 1
            case_idx += 1
